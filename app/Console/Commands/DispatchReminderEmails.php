...
class DispatchWampiriadaMailing extends Command
{
    ...
    public function handle()
    {
        ...
        $action_day_ids = Action::where('day', '<', Carbon::now()->addDays(2))->get()->pluck('id');

        $reminders = Reminder::whereSent(false)->whereIn('action_day_id', $action_day_ids)->get();


        foreach($reminders as $reminder) {
            //XXX ERROR! this should be for action_day
            $composer = new WampiriadaReminderMailingComposer($reminder);
        }
